...
> [`templ.fun` UI](../../../apps/web/src/components/pages/create/lib/defaults.ts)
> pre-fills on the create form, and what the `apps/scripts/src/create-templ.ts`
> CLI mirrors. If you call `Factory.createTempl` directly (e.g. from etherscan
> or your own SDK), you must provide explicit values for every field.
