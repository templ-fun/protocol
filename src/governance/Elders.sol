...
///
///         Before deploying this template to production, the indexer must
///         handle `EldersInitialized` and store the threshold, and the UI
///         must read that value to identify which members are elders
///         (memberId <= threshold).
///
