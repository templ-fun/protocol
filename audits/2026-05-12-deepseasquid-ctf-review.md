...
**The dangerous property** is not that the new member votes in the same batch; it is that the new member's presence lets the batch remove the original sole council member without tripping `EmptyCouncil`. The current priest will already have voted YES on the proposal (otherwise it doesn't execute). The proposal's eviction step then succeeds because `councilSize` was bumped to 2 by the prior add step, and removing one member of a 2-member council does not violate the `councilSize <= 1` guard. After execution, the attacker is left as the sole council member without ever having cast a vote.
...
**(b) Activation delay for new council members.** Add a `joinedAt` field on the `isCouncilMember` mapping (or a separate `councilJoinedAt` map). During the activation delay window, a newly-added council member counts neither (i) for voting nor (ii) for the `councilSize > 1` minimum-active-council-size check that `removeCouncilMember` uses to prevent emptying the council. The latter constraint is what actually closes the attack: in the multi-op batch, the just-added attacker doesn't count toward `councilSize` for the `EmptyCouncil` guard, so step 2 of the batch (removing the original priest) reverts as it would have without the prior add. Once the activation delay passes, the new member becomes fully active for both voting and the size check. This is heavier-handed than option (a) but composable with other governance flows.
...
- **2026-05-12 (same day, post-priest-re-review)** — Second round of revisions per the priest's re-review of the revised PR:
  - F-2 description: Added a "dangerous property" clarification paragraph to explain that the attack mechanism is not the new member voting in the batch, but the new member's mere presence allowing the eviction step to pass the `EmptyCouncil` guard. The current priest is already the one voting yes on the proposal in the first place.
  - F-2 remediation (b): Sharpened the activation-delay option to specify that new council members must not count for the minimum-active-council-size check during the delay window — not just for voting. This is what actually closes the attack. The original "vote rights aren't activated yet" framing was muddled because the new member never needed to vote in the first place.
  - Per priest's bottom-line statement, this round of revisions satisfies the priest's "credible public review note" standard.
